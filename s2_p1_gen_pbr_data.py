...
if __name__ == '__main__':
    ...
    bop_parent_path = os.path.join(current_dir, 'bop_datasets')
    bop_dataset_path = os.path.join(bop_parent_path, dataset_name)

    # Load the 3D model information of the dataset.
    # 加载数据集的 3D 模型信息。
    models_info = load_json2dict(os.path.join(bop_dataset_path, 'models', 'models_info.json'))

    if not os.path.exists(os.path.join(bop_dataset_path, 'camera.json')):
        write_dict2json(os.path.join(bop_dataset_path, 'camera.json'), 
                            {
    ...
    # 直接设置相机分辨率与内参矩阵 K
    cam_info = load_json2dict(os.path.join(bop_dataset_path, 'camera.json'))
    bproc.camera.set_resolution(cam_info['width'], cam_info['height'])
    cam_K = np.array([
        [cam_info['fx'], 0.0, cam_info['cx']],
        [0.0, cam_info['fy'], cam_info['cy']],
        [0.0, 0.0, 1.0]
    ])
    bproc.camera.set_intrinsics_from_K_matrix(cam_K, cam_info['width'], cam_info['height'])
    ...
    # if os.path.basename(cc_textures_path) == 'textures':
    ...
    tex_dir = os.path.join(current_dir, 'resources', 'textures')
    texture_files = glob.glob(os.path.join(tex_dir, '*.jpg')) + glob.glob(os.path.join(tex_dir, '*.png'))
    cc_textures = []
    for tex_path in texture_files:
        mat = bproc.material.create(os.path.basename(tex_path))
        mat.set_principled_shader_value("Base Color", 
                                        bpy.data.images.load(tex_path, check_existing=True))
        cc_textures.append(mat)
    
    def sample_pose_func(obj: bproc.types.MeshObject):
        if np.random.rand() < 0.5:
            # 真正的正立模式
            pitch = float(np.random.uniform(-0.06, 0.06))
            roll = float(np.random.uniform(-0.06, 0.06))
            yaw = float(np.random.uniform(0.0, 2.0 * np.pi))
            obj.set_rotation_euler([pitch, roll, yaw])
            obj.set_location([np.random.uniform(-0.12, 0.12), np.random.uniform(-0.12, 0.12), np.random.uniform(0.032, 0.042)])

        else:
            # 自由摔落
            obj.set_location(np.random.uniform([-0.12, -0.12, 0.35], [0.12, 0.12, 0.55]))
    ...
    # 获取本地 4 张室内真实 HDR 环境贴图
    hdri_files = glob.glob(os.path.join(current_dir, 'resources', 'hdris', '*.hdr'))

    target_bop_objs = bproc.loader.load_obj(os.path.join(current_dir, "dji_action4_refined.obj"))
    ...
    for i in tqdm(range(num_scenes)):
        ...
        for obj in (sampled_target_bop_objs):      
            ...
            try:
                mat.set_principled_shader_value("Specular", np.random.uniform(0.1, 0.5))
            except Exception:
                mat.set_principled_shader_value("Specular IOR Level", np.random.uniform(0.1, 0.5))
        ...
        # 为每个场景随机切换一张真实室内环境 HDR
        if hdri_files:
            bproc.world.set_world_background_hdr_img(np.random.choice(hdri_files))
        ...
        while cam_poses < 10:
            ...
            aim_point = poi + np.array([0.0, 0.0, 0.015])
            # 限制相机翻转
            rotation_matrix = bproc.camera.rotation_from_forward_vec(aim_point - location, inplane_rot=np.random.uniform(-0.5, 0.5))
            cam2world_matrix = bproc.math.build_transformation_mat(location, rotation_matrix)
            if bproc.camera.perform_obstacle_in_view_check(cam2world_matrix, {"min": 0.12}, bop_bvh_tree):
